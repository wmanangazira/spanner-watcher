...
public class SampleData {
  ...
  static final ImmutableList<String> CREATE_TABLE_STATEMENTS =
      ImmutableList.of(
          ...
          "CREATE TABLE Singles (\n"
              ...
              + "  AlbumId INT64,\n"
              + "  SingleId INT64,\n"
              + "  SingleTitle STRING(MAX),\n"
              + "  LastUpdateTime TIMESTAMP NOT NULL OPTIONS (allow_commit_timestamp=true)\n"
              + ") PRIMARY KEY (SingerId, AlbumId, SingleId),\n"
  ...
  /** Class to contain single sample data. */
  static class Single {
    final long singerId;
    final long albumId;
    final long singleId;
    ...
    Single(long singerId, long albumId, long singleId, String singleTitle) {
      this.singerId = singerId;
      this.albumId = albumId;
  ...
  static final List<Single> SINGLES =
      Arrays.asList(
          new Single(1, 1, 1, "Lemonade"),
          new Single(1, 2, 2, "Love Me Two Times"),
          new Single(2, 1, 3, "(Sittin` On) the Dock of the Bay"),
          new Single(2, 2, 4, "Don`t Sweat the Technique"),
          new Single(2, 3, 5, "Poker Face"));
    ...
    try {
      ...
      try (ResultSet rs =
          dbClient
              ...
              .executeQuery(
                  Statement.of(
                      "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME IN ('Singers', 'Albums', 'Singles')"))) {
        if (rs.next()) {
          return;
        }
      }
  ...
  static void writeExampleData(DatabaseClient dbClient) {
    ...
    for (Single single : SINGLES) {
      mutations.add(
          Mutation.newInsertOrUpdateBuilder("Singles")
              .set("SingerId")
              .to(single.singerId)
              .set("AlbumId")
              .to(single.albumId)
